...
module.exports = {
    ...
    run: async(client, interaction, args) => {
        ...
        var statusFull;
        if (interaction.guild.members.cache.filter(member => member.presence?.status == "dnd")) statusFull = '🔴 | DND';
        if (interaction.guild.members.cache.filter(member => member.presence?.status == "offline")) statusFull = '⚫ | Offline';
        if (interaction.guild.members.cache.filter(member => member.presence?.status == "online")) statusFull = '🟢 | Online';
        if (interaction.guild.members.cache.filter(member => member.presence?.status == "idle")) statusFull = '🟡 | Idle';
        var bot = false;

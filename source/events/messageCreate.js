...
module.exports = async(client, message) => {
    ...
    if (lang == "en") {
        if (!admins.includes(message.author.id)) return message.reply({ content: '> :x: **You Are Not From The Allowed Admins**' });
    } else if (lang == "ar") {
        if (!admins.includes(message.author.id)) return message.reply({ content: '> :x: **انت لست من ادمنز البوت**' });
    }

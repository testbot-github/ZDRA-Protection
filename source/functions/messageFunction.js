    ...
    try {
        ...
        if (toggle == "on") {
            ...
            if (
                ...
                if (checker == null) db.set(`WarnsMute_${message.guild.id}`, { data: 1 });
                else db.add(`WarnsMute_${message.guild.id}.data`, 1);
                if (checker > 3) {
                    let role = message.guild.roles.cache.find(r => r.name == "Muted");
                    if (!role) message.roles.guild.create({
                        data: {
                            name: "Muted"
                        }
                    }).then(role => {
                ...
                if (lang == "en") return message.delete().then(() => message.channel.send({ content: `> ⚠️ **Anti-Links Porotecion Is Here <@!${message.author.id}>!!**` }));
                if (lang == "ar") return message.delete().then(() => message.channel.send({ content: `> ⚠️ **حماية الروابط هنا <@!${message.author.id}>!!**` }));
            }
        }
        ...
        if (toggle2 == "on") {
            ...
            if (
                ...
                if (lang == "en") return message.delete().then(() => message.channel.send({ content: `> ⚠️ **Anti-Swear Porotecion Is Here <@!${message.author.id}>!!**` }));
                if (lang == "ar") return message.delete().then(() => message.channel.send({ content: `> ⚠️ **حماية الشتم هنا <@!${message.author.id}>!!**` }));
            }
        }
        ...
        if (message.attachments) {
            ...
            if (message.channel.id == channellll.id) message.delete();
        }
    } catch (err) {
        return;
    }
